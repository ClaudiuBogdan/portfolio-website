...
export const useNotificationsLifecycle = () => {
  ...
  useEffect(() => {
    ...
    const updateNextCloseTime = (closeTime: number | null | undefined) => {
        ...
        if (nextNotificationCloseTime === null || closeTime < nextNotificationCloseTime) {
    ...
    if (nextNotificationCloseTime !== null) {
        const timeout = setTimeout(() => {
            setNotifications((notifications) =>
                notifications.map((notification) => ({
                    ...notification,
                    hide: typeof notification.closeAt === 'number' && notification.closeAt <= Date.now(),
                }))
            );
        }
        , nextNotificationCloseTime - Date.now());
